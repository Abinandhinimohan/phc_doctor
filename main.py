...
def doc_capture():
    ...
    if act=="capture":
        ...
        if image is not None:
            ...
            mycursor.execute("update ph_attendance set geo_image=%s,latitude=%s,longitude=%s,att_time=%s,face_st=%s,geo_st=%s,time_st=%s,att_st=%s where att_date=%s && docid=%s",(dimg,lat,lon,rd[1],fstatus,status,tstatus,att_st,cdate,uname))
            mydb.commit()
            
            
        
    return render_template('web/doc_capture.html', msg=msg,act=act,rs=data,dimg=dimg,mess=mess,st=st,email1=email1,email2=email2,email3=email3,mobile1=mobile1,mobile2=mobile2,m1=m1,uname=uname)
...
def check_loc():
    ...
    return render_template('web/check_loc.html',st=st,mess=mess,mobile1=mobile1,email1=email1,mobile2=mobile2,email2=email2,email3=email3,dst=dst)
